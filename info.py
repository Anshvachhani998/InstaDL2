import os

SESSION = "my_bot"
API_ID = int(os.getenv("API_ID", "8012239"))
API_HASH = os.getenv("API_HASH", "171e6f1bf66ed8dcc5140fbe827b6b08")
BOT_TOKEN = os.getenv("BOT_TOKEN", "6346317908:AAH2nKFgF-MQOOzI_S30PZKNZK_aCoUhDC4")
LOG_CHANNEL = int(os.getenv("LOG_CHANNEL", "-1002284232975"))
FORCE_CHANNEL = int(os.getenv("FORCE_CHANNEL", "-1002284232975"))
DUMP_CHANNEL = int(os.getenv("DUMP_CHANNEL", "-1002284232975"))
PORT = int(os.getenv("PORT", "8080"))
MONGO_URI = os.getenv("MONGO_URI", "mongodb+srv://Ansh089:Ansh089@cluster0.y8tpouc.mongodb.net/?retryWrites=true&w=majority")
MONGO_NAME = os.getenv("MONGO_NAME", "InstaDL2")
ADMINS = [5660839376]
